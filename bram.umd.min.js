 ...
 * Code distributed by Google as part of the polymer project is also
 * subject to an additional IP rights grant found at http:polymer.github.io/PATENTS.txt
 */const K=new Map,L=function(P,Q,R,S=!1){if(null==Q)throw new Error('The first argument of createInstance must be an implementation of TemplateProcessor');(!K.has(P)||S)&&K.set(P,new E(P));const T=K.get(P);return new J(T,Q,R)};class M extends F{constructor(P,Q){super(),Object.assign(this,P),this._state=Q}clear(){e()}applyValue(P){const Q=P.bind(this._state);this.element.addEventListener('click',Q)}}const N=new class extends v{createdCallback(Q,R){for(let S=Q[0],T=0;S;)S instanceof G&&S.rule.attributeName.startsWith('@')&&(Q[T]=new M(S,R)),T++,S=Q[T]}processCallback(Q,R){for(const S of Q)if(S instanceof I);else if(S instanceof H){const{expression:T}=S.rule;S.value=R&&T&&R[T]}else if(S instanceof G){const{expressions:T}=S.rule;S.value=R&&T&&T.map(U=>R&&R[U])}else if(S instanceof M){const{expressions:T}=S.rule;S.value=R&&R[T[0]]}}},O=h(HTMLElement);return h.Element=O,h.model=s,h.on=function(P,Q,R){var S=P[q];if(S){var T=S[Q];T||(T=S[Q]=[]),T.push(R)}},h.off=function(P,Q,R){var S=P[q];if(S){var T=S[Q];if(T){var U=T.indexOf(R);-1===U||(T.splice(U,1),!T.length&&delete S[Q])}}},h.createInstance=f,h});
