 ...
 * Code distributed by Google as part of the polymer project is also
 * subject to an additional IP rights grant found at http:polymer.github.io/PATENTS.txt
 */const templateDefinitionCache=new Map,createInstance=function(a,b,c,d=!1){if(null==b)throw new Error('The first argument of createInstance must be an implementation of TemplateProcessor');(!templateDefinitionCache.has(a)||d)&&templateDefinitionCache.set(a,new TemplateDefinition(a));const e=templateDefinitionCache.get(a);return new TemplateInstance(e,b,c)};function notImplemented(){throw new Error('Not yet implemented')}class EventTemplatePart extends TemplatePart{constructor(a,b,c){super(),Object.assign(this,a),this._state=b,this._thisValue=c}clear(){notImplemented()}applyValue(a){const b=a.bind(this._thisValue||this._state);this.element.addEventListener('click',b)}}class BramTemplateProcessor extends TemplateProcessor{constructor(a){super(),this._thisValue=a}createdCallback(a,b){for(let c=a[0],d=0;c;)c instanceof AttributeTemplatePart&&c.rule.attributeName.startsWith('@')&&(a[d]=new EventTemplatePart(c,b,this._thisValue)),d++,c=a[d]}processCallback(a,b){for(const c of a)if(c instanceof InnerTemplatePart);else if(c instanceof NodeTemplatePart){const{expression:d}=c.rule;c.value=b&&d&&b[d]}else if(c instanceof AttributeTemplatePart){const{expressions:d}=c.rule;c.value=b&&d&&d.map(e=>b&&b[e])}else if(c instanceof EventTemplatePart){const{expressions:d}=c.rule;c.value=b&&b[d[0]]}}}function createInstance$1(a,b={},c){let d=new BramTemplateProcessor(c),e=createInstance(a,d,b);const f=toModel(b,()=>{e.update(f)});return{model:f,fragment:e,update(){e.update(f)}}}function getTemplate(a){return'string'==typeof a?document.querySelector(a):a}function Bram(a){return class extends a{constructor(){super();// Initially an empty object
